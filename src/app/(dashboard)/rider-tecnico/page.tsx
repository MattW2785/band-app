...
export default async function RiderTecnicoPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <Card className="mb-6">
        <h2 className="mb-3 font-medium text-zinc-900">Requisiti generali</h2>
        <TechRiderForm initial={techRider} />
      </Card>

      <Card className="mb-6">
        <h2 className="mb-3 font-medium text-zinc-900">Aggiungi un canale</h2>
        <AddChannelForm />
      </Card>

      <Card>
        <h2 className="mb-3 font-medium text-zinc-900">Channel list</h2>
            ...
            <tbody>
              {channels.map((c) => (
                <tr key={c.id} className="border-t border-zinc-100">
                  <td className="py-1.5 pr-2 text-zinc-800">{c.channel_number ?? "—"}</td>
                  <td className="py-1.5 pr-2 text-zinc-800">{c.source}</td>
                  <td className="py-1.5 pr-2 text-zinc-600">{c.mic_or_di ?? "—"}</td>
                  <td className="py-1.5 pr-2 text-zinc-600">{c.stand ?? "—"}</td>
                  <td className="py-1.5 pr-2 text-zinc-600">{c.notes ?? "—"}</td>
                  <td className="py-1.5">
        ...
        {channels.length === 0 && <p className="text-sm text-zinc-500">Nessun canale aggiunto ancora.</p>}
      </Card>
    </div>
  );
}
