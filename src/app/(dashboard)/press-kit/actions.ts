...
const PressKitSchema = z.object({
  ...
  stage_plot_url: z.string().trim().optional(),
  tech_rider_url: z.string().trim().optional(),
...
export async function updatePressKit(
  ...
  const parsed = PressKitSchema.safeParse({
    ...
    stage_plot_url: formData.get("stage_plot_url"),
    tech_rider_url: formData.get("tech_rider_url"),
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      stage_plot_url: parsed.data.stage_plot_url || null,
      tech_rider_url: parsed.data.tech_rider_url || null,
