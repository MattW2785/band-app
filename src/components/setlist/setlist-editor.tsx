...
function SortableRow({ item, setlistId }: { item: SetlistSongItem; setlistId: string }) {
  ...
  return (
    <li
      ...
      className={`flex items-center justify-between gap-3 rounded-lg border border-zinc-200/80 bg-white px-3 py-2 shadow-sm ${isDragging ? "opacity-60 shadow-md" : ""}`}
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-zinc-900">
            {item.title} {item.artist && <span className="font-normal text-zinc-500">— {item.artist}</span>}
          </p>
      ...
      <form action={removeSongFromSetlist}>
        ...
        <Button type="submit" variant="ghost" className="text-red-500">
          Rimuovi
        </Button>
      </form>
    </li>
...
export function SetlistEditor({
  ...
  return (
    <div>
      <div className="mb-4">
        <div className="mb-1.5 flex items-center justify-between text-sm">
          <span className="text-zinc-600">
            Durata totale <span className="font-medium text-zinc-900">{formatDuration(totalSeconds)}</span>
          </span>
          <span className="text-zinc-500">target {targetMinutes} min</span>
        </div>
      ...
      {items.length === 0 && <p className="text-sm text-zinc-500">Nessun brano in scaletta.</p>}
    </div>
  );
}
